...
output "change_hash" {
  ...
  value = sha1(join(",", flatten([
    tolist([twilio_autopilot_task.task.sid]),
    [for task_field in module.task_fields : task_field.change_hash],
    [for task_sample in twilio_autopilot_task_sample.task_sample : sha1(join(",", tolist([task_sample.sid, task_sample.language, task_sample.tagged_text])))],
  ])))
}
