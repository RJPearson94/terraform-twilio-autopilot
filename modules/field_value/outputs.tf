...
output "change_hash" {
  ...
  value = sha1(join(",", flatten([
    tolist([sha1(join(",", tolist([twilio_autopilot_field_value.field_value.sid, twilio_autopilot_field_value.field_value.language, twilio_autopilot_field_value.field_value.value])))]),
    [for synonym in twilio_autopilot_field_value.synonym_field_value : sha1(join(",", tolist([synonym.sid, synonym.language, synonym.value, synonym.synonym_of])))],
  ])))
}
